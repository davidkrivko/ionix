...
ThermostatEditDialog.propTypes = {
  thermostat: PropTypes.object.isRequired,
  onSubmit: PropTypes.func
};

export default function ThermostatEditDialog({ thermostat, onSubmit }) {
  const isMountedRef = useIsMountedRef();
  const { enqueueSnackbar, closeSnackbar } = useSnackbar();

  const [open, setOpen] = useState(false);

  const ThermostatSchema = Yup.object().shape({
  ...
  const formik = useFormik({
    ...
    validationSchema: ThermostatSchema,
      ...
      await axios
        ...
        .then((response) => {
          ...
          if (response.status === 200) {
            enqueueSnackbar('Thermostat tag was successfully updated.', {
              variant: 'success',
              action: (key) => (
    ...
    <Box sx={{ maxWidth: '100%' }}>
      ...
      <DialogStyle onClose={handleClose} aria-labelledby="customized-dialog-title" open={open}>
        <DialogTitle sx={{ m: 0, p: 2 }}>
          <Typography sx={{ fontSize: '18px', fontWeight: '700' }}>Update Thermostat Name</Typography>
          <IconButton
            aria-label="close"
            onClick={handleClose}
            sx={{
