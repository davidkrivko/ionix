...
<div class="flex flex-col h-full" in:fade>
    ...
    <div class="px-4 md:px-6 py-12 bg-gray-50 rounded-br-2xl flex flex-col flex-grow">
        <div class="mt-2">
            <button on:click={() => support_form = true}
            class="text-sm px-4 py-2 shadow-md active:shadow-sm bg-gray-100 text-gray-700 active:ring-blue-700">New case</button>
        </div>
        {#if success_form}
        ...
        {/if}
        {#if support_form}
            <div class="grid grid-cols-1 rounded-md mt-4 px-6 py-6 bg-white shadow-md max-w-lg" in:slide>
                <div class="flex flex-col gap-y-4">
                    <div class="font-semibold text-gray-700">
                        Subject *:
                    </div>
                    <div class="">
                        <input type="text" class:error={subject === null} bind:value={subject} placeholder="e.g. Heating problem" class="px-2 border-1 w-full h-10 ring-1 ring-gray-600">
                    </div>
                    <div class="font-semibold text-gray-700">
                        Email *:
                    </div>
                ...
                <div class="py-4 flex flex-row flex-wrap gap-4">
                    ...
                    class="text-sm px-4 py-2 shadow-md active:shadow-sm bg-gray-100 text-gray-700 active:ring-blue-700 rounded-md">
                        Submit
                    </button>
        ...
        <div class="pt-10 pb-4">
            <button class="text-sm text-gray-700 ">My requests:</button>
        </div>
        
        {#each requests as request, index }
            <div on:click={() => item_details[index] = !item_details[index]}
            in:slide class="grid grid-cols-3 px-4 py-3 mt-4 ml-2 gap-x-4 shadow-md support rounded-md bg-white text-black cursor-pointer">
            ...
            {#if item_details[index]}
                <div transition:slide class="px-4 py-3 mt-2 ml-2 text-sm text-gray-700 rounded-sm shadow-sm bg-white">
                    {request.request}
                </div>
            {/if}
        {:else}
